...
#variables para mostrar la pantalla de muerte
tiempo_muerte=None
...
while running:
    ...
    # Dentro del bucle del juego (o donde evalúes la vida del protagonista)
    if protagonista.vida == 0:
            tiempo_muerte = pygame.time.get_ticks()

        # Mostrar el mensaje de "HAS MUERTO"
            GameOver_text = font2.render("HAS MUERTO", True, Black)
            screen.blit(GameOver_text, (800 / 3, 600 / 2))
            rectangulos_visibles1 = False
            points_visibles = True

        # Si han pasado 2 segundos (2000 milisegundos) desde la muerte, borra el mensaje
            if pygame.time.get_ticks() - tiempo_muerte > 2000:
                tiempo_muerte = None  # Restablece la variable para el próximo uso
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if rectangulos_visibles:
                ...
                #estos son los botones para seleccionar los "ataques"
...
#aquí va la función para el combate
    def combate(Prota, enemigo1, seleccion_prota, seleccion_enemigo):
        if seleccion_prota==seleccion_enemigo:
            Prota.modificar_vida(-Prota.vida*0.1)
            enemigo1.modificar_vida(-enemigo1.vida*0.1)
        elif (seleccion_prota == "piedra" and seleccion_enemigo == "tijera"):
            enemigo1.modificar_vida(-Prota.daño)
        elif (seleccion_prota == "papel" and seleccion_enemigo == "piedra"):
            enemigo1.modificar_vida(-Prota.daño)
        elif (seleccion_prota == "tijera" and seleccion_enemigo == "papel"):
            enemigo1.modificar_vida(-Prota.daño)
        else:
    ...
    if rectangulos_visibles1:
        ...
        BotonPiedra = pygame.draw.rect(screen, Black, (50, 450, 200, 80))
        BotonPapel = pygame.draw.rect(screen, Black, (300, 450, 200, 80))
        Botontijera = pygame.draw.rect(screen, Black, (550, 450, 200, 80))
    ...
    if points_visibles:
        Nivel_1 = pygame.draw.circle(screen, "purple", (150, 150), 25)
        Nivel_2 = pygame.draw.circle(screen, "purple", (400, 450), 25)
        Nivel_3 = pygame.draw.circle(screen, "purple", (650, 150), 25)
        draw_return_button(button_position_left)
