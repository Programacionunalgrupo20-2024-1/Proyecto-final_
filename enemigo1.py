...
class Villano:
    def __init__(self, x, y):
        self.x = x
        self.y = y
        # Cargar las dos imágenes de animación
        self.sprites = [
            pygame.image.load("sprites/caballero_oscuro1.png"),
            pygame.image.load("sprites/caballero_oscuro2.png")
        ]
        self.current_sprite = 0
        self.altura_deseada = 300  # Ajusta el tamaño deseado
        self.ajustar_imagen()  # Ajustar tamaño de las imágenes iniciales
        self.vida = 100
        self.daño = 20
        self.critico = 30
        self.tiempo_animacion = 200  # Controla la velocidad de animación (200ms entre frames)
        self.ultimo_tiempo = pygame.time.get_ticks()  # Para controlar el tiempo de animación
    ...
    def dibujar_barra_vida(self, screen, font):
        ...
        pygame.draw.rect(screen, color_borde, (self.x + 10, self.y - 20, barra_vida_ancho + 2, barra_vida_alto + 2))
        ...
        pygame.draw.rect(screen, color_vida, (self.x + 11, self.y - 19, int(barra_vida_ancho * vida_porcentaje), barra_vida_alto))

        texto_blanco = font.render(f"{int(self.vida)}% HP", True, (255, 255, 255))
        texto_negro = font.render(f"{int(self.vida)}% HP", True, (0, 0, 0))
        screen.blit(texto_negro, (self.x + 20, self.y - 41))  # Borde negro
        screen.blit(texto_blanco, (self.x + 21, self.y - 40))  # Texto blanco
